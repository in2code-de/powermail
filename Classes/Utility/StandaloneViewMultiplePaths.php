...
/**
 * Extends the regular standalone template view with the possibilities to use multiple paths (like TemplateView is able to use).
 */
class StandaloneViewMultiplePaths extends \TYPO3\CMS\Fluid\View\StandaloneView {
	 ...
	 * @return void
	 * @deprecated Is provided by the StandaloneView-API, but should not be used anymore with this extended API.
	 * @api
	 */
	...
	public function getLayoutRootPaths() {
		if ($this->layoutRootPaths === NULL && $this->templatePathAndFilename === NULL) {
			throw new \TYPO3\CMS\Fluid\View\Exception\InvalidTemplateResourceException('No layout root paths have been specified. Use setLayoutRootPaths().', 1410653161);
		}
	 ...
	 *
	 * @param array $partialRootPath Fluid partial root paths
	 * @return void
	 * @api
	 */
	...
	public function getPartialRootPaths() {
		if ($this->partialRootPaths === NULL && $this->templatePathAndFilename === NULL) {
			throw new \TYPO3\CMS\Fluid\View\Exception\InvalidTemplateResourceException('No partial root paths have been specified. Use setPartialRootPath().', 1410653162);
		}
	...
	protected function getLayoutPathAndFilename($layoutName = 'Default') {
		...
		if (empty($layoutRootPaths)) {
			throw new \TYPO3\CMS\Fluid\View\Exception\InvalidTemplateResourceException('Layout root paths do not exist.', 1410653917);
		}
	...
	protected function getPartialPathAndFilename($partialName) {
		...
		if (empty($partialRootPaths)) {
			throw new \TYPO3\CMS\Fluid\View\Exception\InvalidTemplateResourceException('Partial root paths do not exist.', 1410653918);
		}
