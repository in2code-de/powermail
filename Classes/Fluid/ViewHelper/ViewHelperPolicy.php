...
{
    /**
     * @var array "namespace:method" => true
     */
    private $exact;

    /**
     * @var string[] "namespace:method-prefix" without the trailing "*"
     */
    private $prefixes;

    /**
     * @var array namespace identifiers with a "namespace:*" entry
     */
    private $namespaces;
    ...
    public static function fromIdentifiers(array $identifiers): self
    {
        ...
        foreach ($identifiers as $identifier) {
            ...
            list($namespace, $method) = explode(':', $identifier, 2);
            if ($namespace === '' || $method === '') {
                continue;
            }
    ...
    /**
     * A namespace is relevant if the allowlist holds at least one entry for it. Used to keep the
     * parser from raising an UnknownNamespaceException for namespaces powermail does not evaluate.
     *
     * @param string $namespace
     * @return bool
     */
    public function isNamespaceRelevant(string $namespace): bool
    {
        ...
        foreach (array_keys($this->exact) as $identifier) {
            if (strpos($identifier, $namespace . ':') === 0) {
                return true;
            }
        }
    ...
    public function resolveAllowedClassNames(ViewHelperResolver $resolver): array
    {
        ...
        foreach (array_keys($this->exact) as $identifier) {
            list($namespace, $method) = explode(':', $identifier, 2);
            try {
                $classNames[$resolver->resolveViewHelperClassName($namespace, $method)] = true;
