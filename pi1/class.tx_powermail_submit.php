...
class tx_powermail_submit extends tslib_pibase {
		...
		if ($this->email_send) {
			...
			if (isset($this->sessiondata['FILE']) && $this->conf['upload.']['attachment'] == 1) { // if there are uploaded files AND attachment to emails is activated via constants
				if (is_array($this->sessiondata['FILE']) && $this->subpart == 'recipient_mail') { // only if array and mail to receiver
					$path = $this->div->correctPath($this->conf['upload.']['folder']) . ((!!$this->conf['upload.']['useTitleAsUploadSubFolderName'])
							? $this->cObj->data['tx_powermail_title'] . '/' : '');
	...
	function saveMail() {
		...
		$this->db_values = array(
			...
			'uploadPath' => t3lib_div::dirname($this->conf['upload.']['folder']) . '/' . ((!!$this->conf['upload.']['useTitleAsUploadSubFolderName'])
					? $this->cObj->data['tx_powermail_title'] . '/' : '')
		);
	...
	function emailSettings() {
		...
		$emails = '';
		$this->sendername = '';
		$this->sender = ($this->cObj->data['tx_powermail_sender'] && t3lib_div::validEmail($this->sessiondata[$this->cObj->data['tx_powermail_sender']])
				? $this->sessiondata[$this->cObj->data['tx_powermail_sender']] : $this->noReplyEmail); // email sender (if sender is selected and email exists)
		...
		if ($subpart != 'db') {
			...
			if ($this->conf['debug.']['output'] == 'all' || $this->conf['debug.']['output'] == 'email') { // only if debug output activated via constants
				...
				$debugarray = array(
					...
					'return path' => (t3lib_div::validEmail($this->cObj->cObjGetSingle($this->conf['email.'][$this->subpart . '.']['returnpath'], $this->conf['email.'][$this->subpart . '.']['returnpath.']))
							? $this->cObj->cObjGetSingle($this->conf['email.'][$this->subpart . '.']['returnpath'], $this->conf['email.'][$this->subpart . '.']['returnpath.'])
							: $this->maildata['sender']), // Return path
					...
					'mailcontent (plaintext)' => ($this->conf['emailformat.'][$this->subpart] != 'html' ? $this->div->makePlain($this->mailcontent[$subpart])
							: 'SYSTEM NOTE: Disabled via typoscript')
				);
		...
		if ($this->ok) { // only if spamhook is not set
			if ($this->conf['clear.']['session'] == 1) { // If set in constants // setup
				$GLOBALS['TSFE']->fe_user->setKey('ses', $this->extKey . '_' . ($this->cObj->data['_LOCALIZED_UID'] > 0 ? $this->cObj->data['_LOCALIZED_UID']
						: $this->cObj->data['uid']), array()); // Generate Session without ERRORS
	...
	function clearCaptchaSession() {
		...
		unset($GLOBALS['TSFE']->fe_user->sesData['powermail_' . ($this->cObj->data['_LOCALIZED_UID'] > 0 ? $this->cObj->data['_LOCALIZED_UID']
				: $this->cObj->data['uid'])]['OK']); // clear all OK from Session (used e.g. from recaptcha)
	}
