			...
			if ($GLOBALS['TSFE']->tmpl->setup['config.']['language']) { // if language was set in ts
				...
				if ($res1 !== false) {
					...
					if (!empty($row1['Field'])) { // if there is a localized version of static_info_tables
						...
						$sorting = ", REPLACE( REPLACE( REPLACE( REPLACE( REPLACE( REPLACE( REPLACE( REPLACE( IF($localfield <> '', $localfield, cn_short_en), 'Ä', 'A'), 'Ö', 'O'), 'Ü', 'U'), 'ä', 'a'), 'ö', 'o'), 'ü','u'), 'ß', 's'), 'Å','A') AS sorting, IF($localfield <> '', $localfield, cn_short_en) AS cn_short_merged";
