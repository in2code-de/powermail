...
class tx_powermail_mandatory extends tslib_pibase {
	...
	public function main($conf, $sessionfields, $cObj) {
		...
		$content_item = '';
		$this->error = 0;
		$this->innerMarkerArray = $this->tmpl = $fieldarray = array();
			...
			foreach ($uniquearray as $value) {
				...
				elseif (strtolower($value) == 'ip' && $confarray['disableIPlog'] != 1) { // value == ip AND IP log is not disabled
					$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery( // get any entry with same IP address like current user
						'senderIP',
						'tx_powermail_mails',
						$where_clause = 'pid = ' . ($this->conf['PID.']['dblog'] ? intval($this->conf['PID.']['dblog'])
								: $GLOBALS['TSFE']->id) . ' AND senderIP = "' . $_SERVER['REMOTE_ADDR'] . '"' . tslib_cObj::enableFields('tx_powermail_mails'),
						$groupBy = '',
						$orderBy = '',
						$limit = 1
					);
					...
					if (isset($autoarray[$this->conf['validate.'][$key]['auto']])) { // if regulare expression in $autoarray
						if ($this->sessionfields[str_replace('.', '', $key != '')]) { // if there is a value in the field, which to check
							...
							if (!preg_match($autoarray[$this->conf['validate.'][$key]['auto']], $this->sessionfields[str_replace('.', '', $key)])) { // If check failed
								$this->sessionfields['ERROR'][str_replace(array('.', 'uid'), '', $key)][] = ($this->conf['validate.'][$key]['errormsg']
										? $this->conf['validate.'][$key]['errormsg'] : $this->pi_getLL('error_expression_validation')); // write errormessage
							}
			...
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) { // One loop for every found email
				if (trim($this->sessionfields['uid' . $row['uid']]) != '') {
					...
					if ($this->pi_getFFvalue(t3lib_div::xml2array($row['flexform']), 'validate') == 'validate-alpha-w-umlaut') {
						if (!preg_match('/^[a-zA-Z\x{00C0}-\x{00FF}\s]+$/', $this->sessionfields['uid' . $row['uid']])) {
					...
					if ($this->pi_getFFvalue(t3lib_div::xml2array($row['flexform']), 'validate') == 'validate-alphanum-w-umlaut') {
						if (!preg_match('/^[0-9a-zA-Z\x{00C0}-\x{00FF}\s]+$/', $this->sessionfields['uid' . $row['uid']])) {
	...
	public function captchaCheck() {
		if ( // only if a supported captcha extension is loaded
			...
			$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
				...
				'tx_powermail_fields LEFT JOIN tx_powermail_fieldsets ON tx_powermail_fields.fieldset = tx_powermail_fieldsets.uid LEFT JOIN tt_content ON tx_powermail_fieldsets.tt_content = tt_content.uid',
				$where_clause = 'tx_powermail_fields.formtype = "captcha" AND tx_powermail_fieldsets.tt_content = ' . ($this->cObj->data['_LOCALIZED_UID'] > 0
	...
	private function overwriteSession() {
		if (count($this->sessionfields['OK']) > 0) { // only if min 1 OK value
			$GLOBALS['TSFE']->fe_user->setKey('ses', $this->extKey . '_' . ($this->cObj->data['_LOCALIZED_UID'] > 0 ? $this->cObj->data['_LOCALIZED_UID']
					: $this->cObj->data['uid']), $this->sessionfields); // Generate Session without ERRORS
