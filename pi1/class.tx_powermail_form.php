...
class tx_powermail_form extends tslib_pibase {
	...
	private function form($limit = '') {
		// Configuration
		$this->InnerMarkerArray = $this->OuterMarkerArray = $this->tmpl = array();
		$this->content_item = ''; // init
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			...
			'tt_content = ' . ($this->cObj->data['_LOCALIZED_UID'] > 0 ? $this->cObj->data['_LOCALIZED_UID']
		...
		$res1 = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			'*', // 'uid,title', before
			'tx_powermail_fieldsets',
			$where_clause = 'tt_content = ' . ($this->cObj->data['_LOCALIZED_UID'] > 0 ? $this->cObj->data['_LOCALIZED_UID']
					: $this->cObj->data['uid']) . tslib_cObj::enableFields('tx_powermail_fieldsets'),
			$groupBy = '',
			$orderBy = 'sorting ASC',
			$limit
		);
		if ($res1 !== false) { // If there is a result
			while ($row_fs = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res1)) { // One loop for every fieldset
				...
				$res2 = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
					...
					$where_clause = 'tx_powermail_fieldsets.tt_content = ' . ($this->cObj->data['_LOCALIZED_UID'] > 0 ? $this->cObj->data['_LOCALIZED_UID']
							: $this->cObj->data['uid']) . ' AND tx_powermail_fields.fieldset = ' . $row_fs['uid'] . tslib_cObj::enableFields('tx_powermail_fieldsets') . tslib_cObj::enableFields('tx_powermail_fields'),
					$groupBy = '',
					$orderBy = 'tx_powermail_fieldsets.sorting, tx_powermail_fields.sorting',
					$limit1 = ''
				);
				if ($res2 !== false) { // If there is a result
				...
				$this->InnerMarkerArray['###OWNCSS###'] = htmlspecialchars($row_fs['class']) . ' ';
	...
	private function tabindexArray() {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			...
			$where_clause = 'tx_powermail_fieldsets.tt_content = ' . ($this->cObj->data['_LOCALIZED_UID'] > 0 ? $this->cObj->data['_LOCALIZED_UID']
					: $this->cObj->data['uid']) . tslib_cObj::enableFields('tx_powermail_fieldsets') . tslib_cObj::enableFields('tx_powermail_fields'),
			$groupBy = '',
			$orderBy = 'tx_powermail_fieldsets.sorting, tx_powermail_fields.sorting',
			$limit = ''
		);
		if ($res !== false) { // If there is a result
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) { // One loop for every field
				switch ($row['formtype']) {
					...
					case 'countryselect':
						$array[] = $row['uid'];
						if ($this->pi_getFFvalue(t3lib_div::xml2array($row['flexform']), 'countryzone') == 1) {
							$array[] = $row['uid'] + 100000;
						}
						break;
					default:
	...
	private function multipleLink($add = 0) {
		...
		if (isset($this->piVars['multiple'])) {
			$this->multiple['currentpage'] = $this->piVars['multiple']; // Currentpage
		} else {
			$this->multiple['currentpage'] = 1; // Currentpage = 1 if not set
		}
		...
		if ($add === 1) { // Forward link
			...
			if ($this->multiple['currentpage'] > 1) { // If current fieldset is not the first
				$linkVars = array(
					'parameter' => $GLOBALS['TSFE']->id,
					'returnLast' => 'url',
					'additionalParams' => '&tx_powermail_pi1[multiple]=' . ($this->multiple['currentpage'] + $add) . '&tx_powermail_pi1[mailID]=' . ($this->cObj->data['_LOCALIZED_UID'] > 0
							? $this->cObj->data['_LOCALIZED_UID'] : $this->cObj->data['uid']),
					'useCacheHash' => 1
				);
