...
class tx_powermail_dynamicmarkers extends tslib_pibase {
    ...
    function DynamicTyposcriptMarker($array) {
		if ($this->conf['dynamicTyposcript.'][strtolower($array[1])]) { // If there is a fitting entry in typoscript
			$string = $this->cObj->cObjGetSingle($this->conf['dynamicTyposcript.'][strtolower($array[1])], $this->conf['dynamicTyposcript.'][strtolower($array[1]).'.']); // fill string with typoscript value
		}
